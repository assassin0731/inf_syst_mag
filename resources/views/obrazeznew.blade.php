...
<script>
$(document).ready(function(){
	$(document).on('hover', '.table3 tr', function(){
		$(this).addClass('hover');
	}, function() {
...
<div id="banner10">
  ...
  <table id="myTable" class="table table3 display rabotniktable" style="width:100%; font-size:18px;" cellspacing='0'>
		<thead>
    ...
    $d=Auth::user()->id_otdel;
      ?>
		<?php
		    $obrazeznew=DB::select('select distinct results.id_zayavprods,results.id_zayav from results join pokaz on pokaz.id=results.id_pokaz where (pokaz.id_otdel=? and results.result is NULL)',[$d]);
        $rowid=0;
		    foreach (($obrazeznew) as $obrazeznews) {
          $rowid=$rowid+1;
		        echo "<tr id='$rowid'>";
            echo "<td style='display:none;'>$rowid</td>";
            echo "<td class='btnRab'><span>Заявка №$obrazeznews->id_zayav</span></td>";
		        echo "<td class='btnRab'><span>Образец №$obrazeznews->id_zayavprods</span></td>";
            echo "<td style='display:none;'  data-id={$obrazeznews->id_zayavprods}>$obrazeznews->id_zayavprods</td>";
