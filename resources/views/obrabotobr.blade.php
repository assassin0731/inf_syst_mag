...
<script>
$(document).ready(function(){
	$(document).on('hover', '.table5 tr', function(){
		$(this).addClass('hover');
	}, function() {
		$(this).removeClass('hover');
	});

$(document).on('click', '.table5 tr', function(){

		$('.table5 tr').removeClass('active');
...
<div id="top3">
  <span style="font-size:30px; margin-top:5px;">
    Заявки с обработанными образцами
  </span>
</div>
...
<div id="banner10">
  ...
  <table id="myTable" class="table table5 display rabotniktable" style="width:100%; font-size:18px;" cellspacing='0'>
		<thead>
	...
	<tbody>
		<?php
		    $zayav=DB::select('select distinct zayav_infos.id from zayav_infos join results on results.id_zayav=zayav_infos.id where (results.result IS NOT NULL) order by zayav_infos.id desc');
        $rowid=0;
